...
| 3 | Smoke post-deploy (rutas, headers, redirects, SEO, endpoint) | Workflow o script dispatch; sin secretos en logs | ☐ |
| 4 | `package.json` `0.1.0` ↔ tag `v1.0.0` | **Decisión:** independencia documentada (ADR-0026); no exigir bump npm | ✅ doc |
| 5 | Eliminar `unsafe-eval` de CSP; evaluar nonces/hashes | Evidencia; ADR si Next lo exige | ☐ |
| 6 | Revisar `includeSubDomains` + `preload` HSTS antes de definitivo | Ya en `vercel.json`; validar impacto subdominios | ☐ |
...
| 10 | Scripts `ci:fast` y `ci:full` | Extender `package.json` sin relajar umbrales §8 | ☐ |
